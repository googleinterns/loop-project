...
class ResBlockTemplate():
  ...
  projection_template: None or a tuple of two projection layer
    ...
    channels in the input tensor and `filters` is the number of filters in the
    convolution. 
  bn_1_template: None or a tuple of four templates tensors (gamma, beta,
    moving_mean, moving_variance) for the first batch normalization layer. All
    templates tensors should be of size (N, n_channels), where N is the number
    of templates, n_channels is the number of channels of the layer inputs.
  bn_2_template: None or a tuple of four templates tensors (gamma, beta,
    moving_mean, moving_variance) for the second batch normalization layer. All
    templates tensors should be of size (N, n_channels), where N is the number
    of templates, n_channels is the number of channels of the layer inputs.
  bn_3_template: None or a tuple of four templates tensors (gamma, beta,
    moving_mean, moving_variance) for the third batch normalization layer. All
    templates tensors should be of size (N, n_channels), where N is the number
    of templates, n_channels is the number of channels of the layer inputs.
  """
  def __init__(self, expansion_template=None, depthwise_template=None,
               projection_template=None, bn_1_template=None, bn_2_template=None,
               bn_3_template=None):
  ...
  def get_bn1_template(self):
    ...
    gamma_init = tf.constant_initializer(self.bn_1_template[0])
    beta_init = tf.constant_initializer(self.bn_1_template[1])
    mean_init = tf.constant_initializer(self.bn_1_template[2]),    
    var_init = tf.constant_initializer(self.bn_1_template[3])
    return gamma_init, beta_init, mean_init, var_init

  def get_bn2_template(self):
    """returns the first batch normalization template."""
    if self.bn_2_template is None:
      return 'zeros', 'ones', 'zeros', 'ones'
    gamma_init = tf.constant_initializer(self.bn_2_template[0])
    beta_init = tf.constant_initializer(self.bn_2_template[1])
    mean_init = tf.constant_initializer(self.bn_2_template[2]),    
    var_init = tf.constant_initializer(self.bn_2_template[3])
    return gamma_init, beta_init, mean_init, var_init
  ...
  def get_bn3_template(self):
    ...
    mean_init = tf.constant_initializer(self.bn_3_template[2]),    
    var_init = tf.constant_initializer(self.bn_3_template[3])
    return gamma_init, beta_init, mean_init, var_init
...
class MixtureWeight(tf.keras.layers.Layer):
  ...
  def call(self, inputa):
    return tf.nn.softmax(self.mixture_weights, axis=0)
